...
import { Bell, LayoutDashboard, Clock3, FilePlus2, Gift, User, Settings, Plus, Menu, X, LogOut } from "lucide-react";
...
export default function Dashboard() {
    ...
    const fetchData = async () => {
      try {
        const [statsRes, issuesRes] = await Promise.all([
          fetch(`${apiBase}/api/user/dashboard`, { headers: authHeaders }),
          fetch(`${apiBase}/api/issues/user/${user.id}`, { headers: authHeaders }),
        ]);
        if (statsRes.ok) setStats(await statsRes.json());
        if (issuesRes.ok) { const d = await issuesRes.json(); setUserIssues(d.issues || []); }
  ...
  const statCards = [
    ...
    { title: "Points", value: points, sub: getLevel(points) },
  ];

  const nav = [
    { name: "Dashboard", icon: LayoutDashboard, path: "/user" },
    { name: "My Issues", icon: Clock3, path: "/user/my-issues" },
    { name: "File Issue", icon: FilePlus2, path: "/user/report" },
    { name: "Rewards", icon: Gift, path: "/user/rewards" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 flex text-white">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 h-full w-64 bg-white/10 backdrop-blur-xl border-r border-white/10 flex flex-col z-30
      ...
      <main className="flex-1 overflow-auto min-w-0">
        ...
        <header className="bg-white/5 border-b border-white/10 px-4 sm:px-6 py-4 flex justify-between items-center gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-base sm:text-lg font-bold truncate">{greeting()}, {user?.name?.split(" ")[0] || "—"}</h2>
              <p className="text-xs text-slate-400 hidden sm:block">{new Date().toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" })}</p>
            </div>
          </div>
          <div className="flex gap-2 items-center flex-shrink-0">
            ...
            <button onClick={() => navigate("/user/report")}
              className="px-3 sm:px-4 py-2 rounded-xl bg-gradient-to-r from-violet-500 to-indigo-500 flex items-center gap-2 text-xs sm:text-sm font-medium hover:opacity-90 transition">
              <Plus size={15} />
        ...
        <div className="p-4 sm:p-6 space-y-5 sm:space-y-6">
          ...
          <section className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
            {statCards.map((card, i) => (
              <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
                <p className="text-xs text-slate-400 mb-2 sm:mb-3">{card.sub}</p>
                <h3 className="text-2xl sm:text-3xl font-bold">{loading ? "—" : card.value}</h3>
                <p className="text-xs sm:text-sm text-slate-300 mt-1">{card.title}</p>
              </div>
            ))}
          </section>
          ...
          <section className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
            <div className="xl:col-span-2 bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
              ...
              <div className="space-y-3">
                {loading ? <p className="text-slate-400 text-sm">Loading...</p>
                  : recentIssues.length > 0 ? recentIssues.map((issue, i) => {
                    ...
                    return (
                      <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-3 sm:p-4 flex items-center gap-3">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium flex-shrink-0 ${badge(status)}`}>{status}</span>
                      </div>
                    );
                  }) : <p className="text-slate-400 text-sm">No issues yet.</p>}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
                ...
                <div className="bg-white/5 rounded-xl p-4 text-center">
                  <h2 className="text-3xl sm:text-4xl font-bold">{loading ? "—" : points}</h2>
                  ...
                  <div className="space-y-2">
                    {topCategories.map(([cat, count], i) => (
                      <div key={i} className="flex justify-between items-center bg-white/5 rounded-xl px-3 py-2 text-sm">
                        ...
                        <span className="text-slate-400 text-xs flex-shrink-0 ml-2">{count}</span>
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-white/5 rounded-2xl p-4 sm:p-5 border border-white/10">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: "Issues resolved", pts: stats.resolved * 20, desc: `${stats.resolved} × 20 pts` },
                  { label: "Total earned", pts: points, desc: "Current balance" },
                ].map((row, i) => (
                  <div key={i} className="flex justify-between items-center bg-white/5 rounded-xl px-3 sm:px-4 py-3">
                    <div>
