...
const DB_TO_UI_STATUS = {
  'open': 'REPORTED', 'pending': 'REPORTED',
...
export default function AdminPage() {
    ...
    async function load() {
      try {
        ...
        const [statsData, issuesData, volunteersData] = await Promise.all([
          fetchAdminStats(), fetchAdminIssues(), fetchAdminVolunteers(),
        ]);
        setStats(statsData);
    ...
    try {
      ...
      if (firstOpenIssue) {
        for (const vId of selectedVolunteers) {
          try { await approveVolunteer(firstOpenIssue._id, vId); } catch (_) { }
        }
      }
      const issuesData = await fetchAdminIssues();
      setIssues(issuesData.issues || []);
    } catch (err) { console.error('Deploy error:', err); }
    finally { setDeploying(false); setShowModal(false); setDeployed(true); }
  };
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen" style={{ background: "#0f1117", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* TOPBAR */}
      <div className="topbar px-4 sm:px-6 py-4 flex justify-between items-center gap-3">
        <div>
        ...
        <button onClick={() => setShowModal(true)} className="deploy-btn flex items-center gap-2 text-white px-3 sm:px-5 py-2.5 rounded-xl text-xs sm:text-sm font-semibold whitespace-nowrap">
          <Rocket size={15} />
          <span className="hidden sm:inline">Deploy Volunteers</span>
      ...
      <div className="p-4 sm:p-6 space-y-5 flex-1">
        ...
        {deployed && (
          <div className="banner-success flex items-center gap-3 px-4 py-3">
            ...
            <p className="text-sm text-emerald-300 font-medium">
              {selectedVolunteers.length} Volunteer{selectedVolunteers.length !== 1 ? 's' : ''} deployed
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4">
          {[
            { label: "HOTSPOTS", value: stats?.hotspots ?? 0, color: "#f87171", icon: <Flame size={14} className="text-red-400" />, iconBg: "rgba(239,68,68,0.12)", iconBorder: "rgba(239,68,68,0.18)", sub: "Active zones" },
            { label: "VOLUNTEERS", value: stats?.activeVolunteers ?? 0, color: "#34d399", icon: <UserCheck size={14} className="text-emerald-400" />, iconBg: "rgba(16,185,129,0.12)", iconBorder: "rgba(16,185,129,0.18)", sub: "Registered" },
            { label: "ANIMALS", value: stats?.animalCases ?? 0, color: "#a5b4fc", icon: <PawPrint size={14} className="text-indigo-400" />, iconBg: "rgba(99,102,241,0.12)", iconBorder: "rgba(99,102,241,0.2)", sub: "Open cases" },
          ].map((s, i) => (
            <div key={i} className="stat-card p-3 sm:p-5">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <p className="hidden sm:block" style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", fontWeight: 600, letterSpacing: "0.06em" }}>{s.label}</p>
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center" style={{ background: s.iconBg, border: `1px solid ${s.iconBorder}` }}>
                  {s.icon}
                </div>
              </div>
              {loading ? <div className="skeleton h-7 sm:h-9 w-12 mb-1" /> : (
                <h2 style={{ fontSize: "clamp(20px, 4vw, 34px)", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, color: s.color, lineHeight: 1 }}>
                  {s.value}
                </h2>
              )}
              <p className="hidden sm:block" style={{ fontSize: "11px", color: "rgba(255,255,255,0.25)", marginTop: "6px" }}>{s.sub}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          {/* MAP */}
          <div className="lg:col-span-2 panel p-4 sm:p-5 flex flex-col" style={{ height: "320px" }}>
            <div className="flex justify-between items-center mb-3 sm:mb-4">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full" style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.18)" }}>
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 pulse-dot inline-block"></span>
                <span style={{ fontSize: "11px", color: "#6ee7b7", fontWeight: 500 }}>Live</span>
              </div>
            </div>
          ...
          {/* RECENT REPORTS */}
          <div className="panel p-4 sm:p-5 flex flex-col" style={{ height: "320px" }}>
            ...
            <div className="space-y-3 overflow-y-auto flex-1 pr-1">
              {loading ? [1, 2, 3].map(i => (
                ...
                <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", textAlign: "center", marginTop: "40px" }}>No issues reported yet</p>
              ) : recentIssues.map((issue) => {
                const uiStatus = DB_TO_UI_STATUS[issue.status] || 'REPORTED';
                const s = statusStyle[uiStatus];
                return (
                  <div key={issue._id} className="report-card p-3 sm:p-4">
                    <div className="flex justify-between items-start gap-2 mb-2">
                      <p style={{ fontSize: "13px", fontWeight: 500, color: "rgba(255,255,255,0.85)", lineHeight: 1.4 }} className="line-clamp-2">
                        {issue.title || issue.category}
                      </p>
                      <span className={`text-[10px] px-2 py-1 rounded-full font-semibold whitespace-nowrap flex items-center gap-1 flex-shrink-0 ${s.pill}`}>
                        <span className={`w-1.5 h-1.5 rounded-full inline-block ${s.dot}`}></span>
                        {uiStatus}
                      </span>
                    </div>
                    <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)" }}>{issue.category} • {timeAgo(issue.createdAt)}</p>
                    {issue.status === 'in-progress' && (
                      <button onClick={() => handleResolve(issue._id)} className="flex items-center gap-1 mt-2" style={{ fontSize: "12px", fontWeight: 500, color: "#818cf8" }}>
      ...
      {/* DEPLOY MODAL */}
      {showModal && (
        <div className="fixed inset-0 modal-overlay flex items-end sm:items-center justify-center z-50 p-4">
          <div className="modal-box w-full sm:w-[440px] rounded-2xl p-5 sm:p-7 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "18px", fontWeight: 600, color: "rgba(255,255,255,0.9)" }}>Assign Volunteers</h2>
                <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", marginTop: "3px" }}>Select volunteers to deploy</p>
              </div>
            ...
            <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
              {volunteers.length === 0 ? (
                <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.3)", textAlign: "center", padding: "20px 0" }}>No volunteers registered yet</p>
              ) : volunteers.map((v) => (
                <label key={v._id} className="vol-card flex items-center justify-between px-4 py-3.5 cursor-pointer">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p style={{ fontSize: "13px", fontWeight: 500, color: "rgba(255,255,255,0.85)" }} className="truncate">{v.name}</p>
                      <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.32)" }} className="truncate">
                        {v.type === 'NGO_Affiliated' ? v.ngoName : "Independent"}
                      </p>
                    </div>
                  </div>
                  <input type="checkbox" onChange={() => toggleVolunteer(v._id)} className="w-4 h-4 cursor-pointer accent-indigo-500 rounded flex-shrink-0 ml-2" />
                </label>
              ))}
            </div>
            <div className="flex justify-end gap-3 mt-6 pt-5" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              <button onClick={handleDeploy} disabled={selectedVolunteers.length === 0 || deploying}
