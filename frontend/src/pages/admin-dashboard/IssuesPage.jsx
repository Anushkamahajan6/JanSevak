...
const statusStyle = {
  'open': { bg: "rgba(245,158,11,0.15)", border: "rgba(245,158,11,0.3)", color: "#fbbf24" },
  'pending': { bg: "rgba(245,158,11,0.15)", border: "rgba(245,158,11,0.3)", color: "#fbbf24" },
  'in-progress': { bg: "rgba(59,130,246,0.15)", border: "rgba(59,130,246,0.3)", color: "#60a5fa" },
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen" style={{ background: "#0f1117", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .filter-btn{border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:5px 10px;font-size:12px;font-weight:500;transition:all 0.15s;cursor:pointer;}
      ...
      <div className="topbar px-4 sm:px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "17px", fontWeight: 600, color: "rgba(255,255,255,0.9)" }}>All Issues</h1>
            <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", marginTop: "2px" }}>{issues.length} total</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {['all', 'open', 'pending', 'in-progress', 'resolved'].map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`filter-btn ${filter === f ? 'active' : ''}`}>
              ...
              {f !== 'all' && <span className="ml-1 opacity-60">{issues.filter(i => i.status === f).length}</span>}
            </button>
          ))}
        </div>

        {error && (
          <div className="px-4 py-3 rounded-xl text-sm text-red-300" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)" }}>{error}</div>
        )}

        <div className="panel">
          {loading ? [1, 2, 3, 4].map(i => (
            <div key={i} className="issue-row flex justify-between items-center py-4 px-4 sm:px-5">
              <div className="space-y-2"><div className="skeleton h-4 w-36 sm:w-48" /><div className="skeleton h-3 w-24 sm:w-32" /></div>
              <div className="skeleton h-7 w-20 sm:w-24 rounded-full" />
            </div>
          )) : filtered.length === 0 ? (
            <div className="py-12 text-center" style={{ color: "rgba(255,255,255,0.3)", fontSize: "13px" }}>No issues found</div>
          ) : filtered.map((issue) => {
            const s = statusStyle[issue.status] || statusStyle['open'];
            return (
              <div key={issue._id} className="issue-row flex justify-between items-center py-4 px-4 sm:px-5 gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {updating === issue._id ? (
                    ...
                    <div className="relative flex items-center" style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: "999px", paddingRight: "6px" }}>
                      <select value={issue.status} onChange={(e) => handleStatusChange(issue._id, e.target.value)} className="status-select" style={{ color: s.color }}>
                        {STATUS_OPTIONS.map(opt => (
                          <option key={opt} value={opt} style={{ background: "#13141f", color: "#fff" }}>{opt.toUpperCase()}</option>
                        ))}
                      </select>
